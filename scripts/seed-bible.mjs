...
const translationId = "ind_ayt";
const translationName = "Alkitab Yang Terbuka";
...
async function main() {
  ...
  console.log(`Reading ${translationId} books from HelloAO...`);
  const bibleApi = new FreeUseBibleApi();
  const translationBooks = await bibleApi.getTranslationBooks(translationId);
  ...
  const books = limitBooks
    ? translationBooks.books.slice(0, limitBooks)
  ...
  async function commitIfNeeded(force = false) {
    ...
    await batch.commit();
    total += pending;
    console.log(`Seeded ${total} verses...`);
    batch = db.batch();
    pending = 0;
  }

  for (const book of books) {
    const chapterCount = limitChapters
      ? Math.min(limitChapters, book.numberOfChapters)
    ...
    for (const chapterNumber of chapterNumbers) {
      ...
      for (const verse of verses) {
        ...
        const id = `${translationId}-${book.id}-${chapterNumber}-${verse.number}`;
  ...
  console.log(`Done. Seeded ${total} Indonesian Bible verses.`);
}
